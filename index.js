...
function createBotApp(env) {
  ...
  const client = neon(env.DATABASE_URL || env.DATABASE);
  ...
  const bot = new Bot(env.BOT_TOKEN); // ⚠️ تعديل: env بدلاً من process.env

  // --- Compatibility layer: simple session, Markup, and Telegraf-like helpers ---
  const _sessions = new Map();
  bot.use(async (ctx, next) => {
    const id = (ctx.from && ctx.from.id) || (ctx.chat && ctx.chat.id) || 'anon';
    if (!_sessions.has(id)) _sessions.set(id, {});
    ctx.session = _sessions.get(id);
    // Telegraf aliases used in the original file
    ctx.replyWithHTML = (text, extra) => ctx.reply(text, Object.assign({}, extra, { parse_mode: 'HTML' }));
    ctx.replyWithMarkdown = (text, extra) => ctx.reply(text, Object.assign({}, extra, { parse_mode: 'Markdown' }));
    ctx.answerCbQuery = (text, extra) => {
      // ctx.answerCallbackQuery exists in grammy; delegate
      if (typeof ctx.answerCallbackQuery === 'function') return ctx.answerCallbackQuery(text, extra);
      return ctx.api.answerCallbackQuery(ctx.callbackQuery && ctx.callbackQuery.id, { text, ...extra });
    };
    await next();
  });

  const Markup = {
    keyboard: (kb, opts = {}) => ({ reply_markup: Object.assign({ keyboard: kb, resize_keyboard: true }, opts) }),
    inlineKeyboard: (buttons, opts = {}) => ({ reply_markup: Object.assign({ inline_keyboard: buttons }, opts) }),
    button: {
      url: (text, url) => ({ text, url }),
      callback: (text, data) => ({ text, callback_data: data }),
    },
    removeKeyboard: (selective = false) => ({ reply_markup: { remove_keyboard: true, selective } })
  };

  // add simple hears/action helpers to mimic Telegraf API used in the original code
  bot.hears = (pattern, handler) => {
    bot.on('message', async (ctx) => {
      const txt = ctx.message && (ctx.message.text || ctx.message.caption || '');
      if (!txt) return;
      let matched = false;
      if (typeof pattern === 'string') matched = txt === pattern;
      else if (pattern instanceof RegExp) matched = pattern.test(txt);
      else if (Array.isArray(pattern)) matched = pattern.includes(txt);
      if (matched) await handler(ctx);
    });
  };
  ...
  bot.action = (pattern, handler) => {
    bot.on('callback_query', async (ctx) => {
      ...
      let matched = false;
      if (typeof pattern === 'string') matched = data === pattern;
      else if (pattern instanceof RegExp) matched = pattern.test(data);
      else if (Array.isArray(pattern)) matched = pattern.includes(data);
      if (matched) await handler(ctx);
    });
  };
  ...
  bot.command('admin', async (ctx) => {
    ...
    const adminId = String(env.ADMIN_ID); // ⚠️ تعديل: env بدلاً من process.env
...
🔹 يمكنك تعديل طلبك أو المحاولة لاحقاً.`
  ...
  postbackApp.get('/', (c) => {
    return c.text('✅ Bot + Postback running on same link');
  });

  postbackApp.get('/callback', async (c) => {
    const { user_id, amount, transaction_id, secret, network } = c.req.query();
    if (secret !== env.CALLBACK_SECRET) { // ⚠️ تعديل: env بدلاً من process.env
    ...
    try {
      ...
      if (existing.rows.length > 0) {
        ...
        return c.text('Duplicate transaction ignored');
      }
      await client.query(
      ...
      return c.text('تمت المعالجة بنجاح');
    } catch (err) {
  ...
  // ==================== التشغيل النهائي ====================
  // بدء تشغيل البوت
  // Running in Cloudflare Workers mode — no bot.launch() here. Updates are handled via the webhook route.
  ...
  // نعيد التطبيقات لاستخدامها في دالة `fetch` الرئيسية
...
// --- END CLOUDFLARE WORKER ENTRY POINT ---
