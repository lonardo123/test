...
`;
          buttons.push([{ text: "📌 قدّم الآن", callback_ `apply_${t.id}` }]);
          ...
          if (t.applied_at) {
            ...
            if (now >= deadline) {
              ...
              buttons.push([{ text: "📝 إرسال إثبات", callback_ `submit_${t.id}` }]);
            } else {
...
${sub.proof}`;
        await ctx.replyWithHTML(text, {
          reply_markup: {
            inline_keyboard: [
              [
                { text: "✅ موافقة", callback_ `approve_${sub.id}` },
                { text: "❌ رفض", callback_ `deny_${sub.id}` }
              ]
            ]
          }
        });
